...
class lGrid
{
	public:
		...
		long getValue( int Column, int Row );		
		void setValue( int Column, int Row, long Value );
		bool inRam();		
		long maximum();
		long minimum();
		long LastErrorCode();
	...
	private:
		...
		void dealloc();
		void alloc();		
		bool inGrid( long column, long row );
		long getValueDisk( int column, int row );
		void setValueDisk( int column, int row, long value );
