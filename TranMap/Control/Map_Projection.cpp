...
CGeoProjection* CMapView::GetMapToWgs84Transform() 
{ 
	VARIANT_BOOL vb;
	_projection->get_HasTransformation(&vb);
	return vb ?  _projection : NULL; 
}
CGeoProjection* CMapView::GetWgs84ToMapTransform() 
{ 
	VARIANT_BOOL vb;
	_wgsProjection->get_HasTransformation(&vb);
	return vb ?  _wgsProjection : NULL; 
}
CGeoProjection* CMapView::GetTilesToMapTransform() 
{ 
	VARIANT_BOOL vb;
...
CGeoProjection* CMapView::GetMapToTilesTransform() 
{
	VARIANT_BOOL vb;
...
CGeoProjection* CMapView::GetWgs84Projection() { return _wgsProjection; }
CGeoProjection* CMapView::GetGMercProjection() { return _gmercProjection; }
CGeoProjection* CMapView::GetMapProjection() { return _projection; }
...
void CMapView::ReleaseProjections()
{
	...
	delete  _tileProjection;

	_tileReverseProjection->Clear(&vb);
	delete _tileReverseProjection;
	
	_wgsProjection->Clear(&vb);
	delete _wgsProjection;
	
	_gmercProjection->Clear(&vb);
	delete _gmercProjection;
}
...
{
	_tileProjection = new CGeoProjection();
	_tileReverseProjection = new CGeoProjection();
	_wgsProjection = new CGeoProjection();
	_gmercProjection = new CGeoProjection();
...
//		SetGeoProjection()
// *****************************************************
void CMapView::SetGeoProjection(CGeoProjection* pVal)
{
	...
	CExtents* ext = GetGeographicExtents();	// try to preserve extents
	...
	if (!isEmpty)
	{
		...
		CExtents* box = ExtentsHelper::GetWorldBounds();
...
//		GetProjection()
// *****************************************************
CGeoProjection* CMapView::GetGeoProjection (void)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	return _projection;
}
...
//		GeodesicLength()
// ***************************************************************
DOUBLE CMapView::GeodesicLength(CShape* polyline)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (!polyline)
	{
...
//		GeodesicArea()
// ***************************************************************
DOUBLE CMapView::GeodesicArea(CShape* polygon)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (!polygon)
	{
...
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	CGeoProjection* gp = GetMapToWgs84Transform();
	if (!gp) {
		ErrorMessage(tkFAILED_TRANSFORM_MAP_TO_WGS84);
		return 0.0;
	}
