...
#pragma region Numbers
	...
	{
		// as long as global locale set to std::locale("C") in TranMap.cpp
...
#pragma endregion
	...
	void WriteXmlHeaderAttributes(CPLXMLNode* psTree, CString fileType)
	{
		...
		CPLString s = CPLString().Printf("%d", m_globalSettings.xmlFileVersion);
	...
	// ****************************************************************** 
	void ClearShapefileModifiedFlag(CShapefile* sf)
	{
		if (!sf) return;
		long numShapes = 0;
	...
	//		GetTranMapPath
	// ************************************************
	CStringW GetTranMapPath()
	{
		wchar_t* path = new wchar_t[MAX_PATH + 1];
		// lu GetModuleFileNameW(GetModuleInstance(), path, MAX_PATH);
		CStringW pathW = path;
		delete[] path;
		return pathW;
	}
