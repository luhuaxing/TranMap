...
public:
	...
	void FromLatLngToXY(PointLatLng pnt, int zoom, CPointClass &ret)
	{
		FromLatLngToXY(pnt.Lat, pnt.Lng, zoom, ret);
	}
	void FromXYToLatLng(CPointClass pnt, int zoom, PointLatLng &ret) 
	{
		FromXYToLatLng(pnt.x, pnt.y, zoom, ret);
	}

	void FromLatLngToXY(double lat, double lng, int zoom, CPointClass &ret)
	{
