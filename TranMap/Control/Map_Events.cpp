...
void CMapView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	switch(m_cursorMode)
	{
		case cmEditShape:
			{
				if (!VertexEditor::OnMouseDown((IMapViewCallback*)this, _shapeEditor, projX, projY, ctrl))
				{
...
void CMapView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	switch(operation)
	{
		case DragRotateShapes:
			{
				CShapefile* sf = _dragging.Shapefile;
				if (sf)
				{
...
// ************************************************************
Extent CMapView::GetPointSelectionBox(CShapefile* sf, double xProj, double yProj)
{
...
void CMapView::DisplayPanningInertia( CPoint point )
{
	if (HasDrawingData(PanningInertia))
	{
		...
		if (inertia)
		{
			...
			double coeff = 1.5 + ((double)_viewWidth * (double)_viewHeight) / 1e6 * 0.7;	
...
{
	//lu CWnd::OnResetState();  // Resets defaults found in DoPropExchange
