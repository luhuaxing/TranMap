...
//	   FindNewShapeID()
// *****************************************************************
long FieldHelper::FindNewShapeID(CShapefile* sf, long FieldIndex)
{
	if (!sf) return 0;
...
bool FieldHelper::UniqueFieldNames(CShapefile* sf)
{
	bool result = false;
	CTableClass* table = NULL;
	sf->get_Table(&table);
	if (table) {
		result = table->MakeUniqueFieldNames();
	}
	return result;
}
...
//		FieldsAreEqual()
// ****************************************************************
bool FieldHelper::FieldsAreEqual(CField* field1, CField* field2)
{
	if (!field1 || !field2)
		return false;
