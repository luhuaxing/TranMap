...
ClipperLib::Polygons* ClipperConverter::Shape2ClipperPolygon(CShape* shp)
{
	if (!shp) 
		return NULL;
...
CShape* ClipperConverter::ClipperPolygon2Shape(ClipperLib::Polygons* polygon)
{
...
//		ClipPolygon()
// ******************************************************************
CShape* ClipperConverter::ClipPolygon(CShape* shapeClip, CShape* shapeSubject, PolygonOperation operation)
{
	...
	if (poly1 && poly2)
	{
		...
		if (result)
		{
			CShape* shp = ogr.ClipperPolygon2Shape(result);
			delete result;
			return shp;
		}
	}
...
// ***************************************************
void ClipperConverter::AddPolygons(CShapefile* sf, ClipperLib::Clipper& clp, ClipperLib::PolyType clipType, bool selectedOnly)
{
	if (!sf) return;
