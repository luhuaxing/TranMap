...
CString ReferenceCounter::GetReport(bool unreleasedOnly)
{
	...
	for(int i = 0; i < INTERFACES_COUNT; i++)
	{
		if (unreleasedOnly)
		{
			...
			{
				temp.Format("Class %s: %d\n", "InerfaceName"/*ComHelper::GetInterfaceName((tkInterface)i)*/, referenceCounts[i]);
				s += temp;
			}
		}
		else
		{
			if (totalCounts[i] != 0)	
			{
				temp.Format("Class %s: %d/%d\n", "InerfaceName"/*ComHelper::GetInterfaceName((tkInterface)i)*/, referenceCounts[i], totalCounts[i]);
				s += temp;
			}
		}
	}
