...
VARIANT_BOOL CMapView::LoadMapState(LPCTSTR Filename, LPDISPATCH Callback)
{
	...
	try {
		...
		if (root)
		{
			CStopExecution* cb = NULL;
			if (Callback)
			{
				// lu Callback->QueryInterface(IID_IStopExecution, (void**)&cb);
			}
...
// ******************************************************************
bool CMapView::DeserializeMapStateCore(CPLXMLNode* node, CStringW ProjectName, VARIANT_BOOL LoadLayers, CStopExecution* callback)
{
	if (!node)
	{
	...
	//lu _shapeDrawingMethod = (s != "") ? (tkShapeDrawingMethod)atoi(s) : dmNewSymbology;
...
BSTR CMapView::SerializeMapState(VARIANT_BOOL RelativePaths, LPCTSTR BasePath)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	USES_CONVERSION;
	CPLXMLNode* node = SerializeMapStateCore(RelativePaths, A2W(BasePath));
	BSTR bstr;
...
CPLXMLNode* CMapView::SerializeMapStateCore(VARIANT_BOOL RelativePaths, CStringW ProjectName)
{
	...
	if (psTree) 
	{
		...
		if (psState)
		{
			...
			// lu if (_shapeDrawingMethod != dmNewSymbology)
			...
			CGeoProjection* gp = GetMapProjection();
