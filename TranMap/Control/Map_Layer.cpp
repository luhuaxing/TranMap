...
#include "OgrHelper.h"
#include "APIHelpers\ProjectionHelper.h"
...
//		GetGetObject()
// ************************************************************
ILayer* CMapView::GetGetObject(long LayerHandle)
{
	if( IsValidLayer(LayerHandle) )
	{	
		...
		ILayer* obj = _allLayers[LayerHandle]->object;
		if (obj != NULL);
		return obj;
	}
	else
...
long CMapView::AddLayerFromDatabase(LPCTSTR ConnectionString, LPCTSTR layerNameOrQuery, VARIANT_BOOL visible)
{
	...
	if (layer) {
		// lu long handle = AddLayer(layer, visible);
		delete layer;
	//	return handle;
	}
	else {
...
// ***************************************************************
void CMapView::AttachGlobalCallbackToLayers(ILayer* object)
{
	...
	// lu object->QueryInterface(IID_CShapefile, (void**)&ishp);
...
//		AddLayer()
// ***************************************************************
long CMapView::AddLayer(ILayer* Object, BOOL pVisible)
{
	long layerHandle = -1;
...
//		AddSingleLayer()
// ***************************************************************
long CMapView::AddSingleLayer(ILayer* Object, BOOL pVisible)
{
	long layerHandle = -1;
	CShapefile *ishp(NULL);
	CGrid *igrid(NULL);
	...
	ishp = dynamic_cast<CShapefile*>(Object);
	iimg = dynamic_cast<CImageClass*>(Object);
	igrid = dynamic_cast<CGrid*>(Object);
	iogr = dynamic_cast<COgrLayer*>(Object);
	...
	if (igrid != NULL)
	{
		...
		if (gridColorScheme)
		{
			...
			if (iimg)
			{
				...
				CGridColorScheme* newScheme = new CGridColorScheme();
...
VARIANT_BOOL CMapView::LoadOgrStyle(Layer* layer, long layerHandle, CStringW name, bool reportError)
{
	...
	CStringW xml = ogrLayer->LoadStyleXML(L"");
	if (xml.GetLength() == 0)
	{
		if (reportError)
...
void CMapView::GrabLayerProjection( Layer* layer )
{
	...
	if (_grabProjectionFromData && layer)
	{
		...
		if (isEmpty)
		{
			CGeoProjection* gp =  layer->GetGeoProjection();
			if (gp)
			{
				gp->get_IsEmpty(&isEmpty);
				if (!isEmpty)
				{
					CGeoProjection* newProj = NULL;
					gp->Clone(&newProj);
					if (!newProj)
					{
...
{
	CGeoProjection* gpMap = GetMapProjection();
...
bool CMapView::ReprojectLayer(Layer* layer, int layerHandle)
{
	...
	if (layer->type == OgrLayerSource)
	{
		...
		if (ogr) {
			ogr->InjectShapefile(sfNew);
		}
	}
	else
	{
		// lu layer->object = sfNew;
	}
...
int CMapView::DeserializeLayerCore(CPLXMLNode* node, CStringW ProjectName, bool utf8Filenames, CStopExecution* callback)
{
	...
	if (layerType == ShapefileLayer)
	{
		...
		if (sf) 
		{
			...
			if (vb)
			{
				// lu layerHandle = this->AddLayer(sf, (BOOL)visible);
	...
	else if (layerType == ImageLayer)
	{
		...
		if (vb)
		{
			// lu layerHandle = this->AddLayer(img, (BOOL)visible);
			delete img;
	...
	else if (layerType == OgrLayerSource)
	{
		...
		if (layer) 
		{
			...
			if (nodeOgrLayer)
			{
				...
				// lu layerHandle = AddLayer(layer, (BOOL)visible);
			}
			delete layer;
		}
	}
...
BSTR CMapView::SerializeLayerOptions(LONG LayerHandle)
{
	...
	CString str = "";
	CPLXMLNode* nodeLayer = SerializeLayerCore(LayerHandle, L"");
	if (nodeLayer)
	{
...
{
	CPLXMLNode* node = this->SerializeLayerCore(layerHandle, L"");
	if (node)
	{
		CPLXMLNode* psTree = CPLCreateXMLNode(NULL, CXT_Element, "TranMap");
		if (psTree)
		{
...
VARIANT_BOOL CMapView::LoadLayerOptionsCore(CString baseName, LONG LayerHandle, LPCTSTR OptionsName, BSTR* Description)
{
	...
	if (root)
	{
		if (_stricmp(root->pszValue, "TranMap") != 0)
		{
			ErrorMessage(tkINVALID_FILE);
		}
		else
