...
//	Draw()				          
//*******************************************************************
int CShapefileDrawer::Draw(const CRect & rcBounds, CShapefile* sf)
{
	if (!sf) return 0;
	...
	// --------------------------------------------------------
	// default options
	CShapeDrawingOptions* iDefOpt = NULL;
	...
	{
		CShapeDrawingOptions* iSelOpt = NULL;
	...
	// categories
	CShapefileCategories* icategories = NULL;
	...
	if (SysStringLen(expr) > 0)
	{
		...
		USES_CONVERSION;
		if (tbl->QueryCore(OLE2CA(expr), arr, err))
		{
			useAll = false;
		}
	}
...
void CShapefileDrawer::DrawPolylinePath(Gdiplus::GraphicsPath* path, CDrawingOptionsEx* options, bool drawSelection)
{
	...
	CLinePattern* pattern = options->linePattern;
