...
void CMapView::DrawImageLayer(const CRect& rcBounds, Layer* l, Gdiplus::Graphics* graphics,	CImageDrawer& imgDrawer)
{
	...
	if (_canUseImageGrouping && iimg->m_groupID != -1)
	{
		// this is grouped image, if this is the first image of group, we shall draw the whole group
		if (!(*_imageGroups)[iimg->m_groupID]->wasDrawn)
		{
			this->DrawImageGroups(rcBounds, graphics, iimg->m_groupID);
			(*_imageGroups)[iimg->m_groupID]->wasDrawn = true;
		}
	}
	else
	{
		...
		if (saveBitmap)
		{
			ScreenBitmap* bmp = iimg->_screenBitmap;
			bool wasDrawn = false;
			...
			if (!wasDrawn)
			{
				...
				else
				{
					...
					if (iimg->_screenBitmap)
					{
						delete iimg->_screenBitmap;
						iimg->_screenBitmap = NULL;
					}

					iimg->_screenBitmap = bmp;
				}
			}
		}
		else
