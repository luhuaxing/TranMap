...
void CMapView::DrawLists(const CRect & rcBounds, Gdiplus::Graphics* graphics, tkDrawReferenceList listType)
{
	...
	std::vector<CLabels*> topLabels;	// labels that should be drawn above all layers
...
CLabels* CMapView::GetDrawingLabels(long DrawingLayerIndex)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CMapView::SetDrawingLabels(long DrawingLayerIndex, CLabels* newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if( newVal == NULL )
	{	
	...
	if( IsValidDrawList(DrawingLayerIndex))
	{	
		_allDrawLists[DrawingLayerIndex]->m_labels = newVal;
	}
	else
		ErrorMessage(tkINVALID_DRAW_HANDLE);
}
