...
//		Validate()
// *****************************************************************
void  CShapefile::Validate(tkShapeValidationMode validationMode, VARIANT_BOOL selectedOnly, CShapeValidationInfo** results)
{
...
//		ValidateInput()
// ********************************************************************
bool CShapefile::ValidateInput(CShapefile* isf, CString methodName, 
	CString parameterName, VARIANT_BOOL selectedOnly, CString className /*= "Shapefile"*/)
{
	CShapeValidationInfo* info = ValidateInputCore(isf, methodName, parameterName, selectedOnly, m_globalSettings.inputValidation, className);
...
//		ValidateInputCore()
// ********************************************************************
CShapeValidationInfo* CShapefile::ValidateInputCore(CShapefile* isf, CString methodName, 
	CString parameterName, VARIANT_BOOL selectedOnly, tkShapeValidationMode validationMode, CString className, bool reportOnly)
{
	...
	CShapeValidationInfo* iinfo = ShapeValidator::Validate(isf, validationMode, svtInput,
...
//		ValidateOutput()
// ********************************************************************
CShapeValidationInfo* CShapefile::ValidateOutput(CShapefile** isf, CString methodName, CString className, bool abortIfEmpty)
{
	if (!(*isf)) return NULL;
...
clear_result:		
	...
	{
		CShapeValidationInfo* iinfo = ShapeValidator::Validate(*isf, m_globalSettings.outputValidation, svtOutput, 
...
//		ValidateOutput()
// **************************************************************
bool CShapefile::ValidateOutput(CShapefile* sf, CString methodName, CString className, bool abortIfEmpty)
{
	if (!_isEditingShapes)
...
void CShapefile::ClearValidationList()
{
	...
	for(size_t i = 0; i < _shapeData.size(); i++)
	{
		...
		{
			delete _shapeData[i]->fixedShape;
...
//		SetValidatedShape
// *********************************************************
void CShapefile::SetValidatedShape(int shapeIndex, ShapeValidationStatus status, CShape* shape)
{
	if (status == Original)
	{
	...
	if (_useValidationList)
	{
		if (_shapeData[shapeIndex]->fixedShape) {
			delete _shapeData[shapeIndex]->fixedShape;
	...
	else
	{
		...
		if (_shapeData[shapeIndex]->shape) delete _shapeData[shapeIndex]->shape;
...
//		GetValidatedShape
// *********************************************************
HRESULT CShapefile::GetValidatedShape(int shapeIndex, CShape** retVal)
{
	if (_useValidationList)
	{
		CShape* shp = _shapeData[shapeIndex]->fixedShape;
		//if (shp) shp->AddRef();
		*retVal = shp;
	}
	else
...
// *********************************************************
void  CShapefile::ClearCachedGeometries()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
