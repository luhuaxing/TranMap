...
//		GetUndoList
// ************************************************************
CUndoList* CMapView::GetUndoList()
{
...
void CMapView::RegisterGroupOperation(DraggingOperation operation)
{
	...
	CShapefile* source = GetShapefile(layerHandle);
	if (source) 
	{
...
bool CMapView::InitDraggingShapefile()
{
	...
	{
		CShapefile* sf = GetShapefile(_dragging.LayerHandle);
		if (sf)
		{
			CShapefile* sfNew = ShapefileHelper::CloneSelection(sf);
			ShpfileType shpType;
...
// ***************************************************************
void CMapView::_UnboundShapeFinished(CShape* shp)
{
	if (!shp) return;
