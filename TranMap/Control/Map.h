...
public:	
	...
	afx_msg ILayer* GetGetObject(long LayerHandle);
	...
	afx_msg CGridColorScheme* GetColorScheme(long LayerHandle);
	...
	afx_msg CExtents* GetExtents();
	afx_msg void SetExtents(CExtents* newValue);
	...
	afx_msg CLabels* GetLayerLabels(LONG LayerHandle);
	afx_msg void SetLayerLabels(LONG LayerHandle, CLabels* pVal);

	afx_msg COgrLayer* GetOgrLayer(LONG LayerHandle);
	afx_msg void SetOgrLayer(LONG LayerHandle, CShapefile* pVal);

	afx_msg CShapefile* GetShapefile(LONG LayerHandle);
	afx_msg void SetShapefile(LONG LayerHandle, CShapefile* pVal);

	afx_msg CImageClass* GetImage(LONG LayerHandle);
	afx_msg void SetImage(LONG LayerHandle, CImageClass* pVal);

	afx_msg BSTR GetLayerName(LONG LayerHandle);
	afx_msg void SetLayerName(LONG LayerHandle, LPCTSTR newVal);

	afx_msg CGeoProjection* GetGeoProjection(void);
	afx_msg void SetGeoProjection(CGeoProjection* pVal);
	...
	afx_msg CExtents* GetGeographicExtents();
	afx_msg VARIANT_BOOL SetGeographicExtents(CExtents* extents);

	afx_msg CTiles* GetTiles(void);
	afx_msg CFileManager* GetFileManager(void);
	afx_msg CIdentifier* GetIdentifier(void);
	...
	afx_msg CLabels* GetDrawingLabels(long DrawingLayerIndex);
	afx_msg void SetDrawingLabels(long DrawingLayerIndex, CLabels* newValue); //LPDISPATCH newValue);
	...
	afx_msg CImageClass* GetUDPointType(long LayerHandle);
	...
	afx_msg void Redraw();
	afx_msg long AddLayer(ILayer* Object, BOOL Visible);
	...
	afx_msg CImageClass* SnapShot(LPDISPATCH BoundBox);
	...
	afx_msg BOOL SnapShotToDC2(PVOID hdc, CExtents* Extents, LONG Width, float OffsetX, float OffsetY, float ClipX, float ClipY, float clipWidth, float clipHeight);
	afx_msg BOOL SnapShotToDC(PVOID hdc, CExtents* Extents, LONG Width);
	afx_msg void LoadTilesForSnapshot(CExtents* Extents, LONG WidthPixels, LPCTSTR Key, tkTileProvider provider);
	afx_msg INT TilesAreInCache(CExtents* Extents, LONG WidthPixels, tkTileProvider provider);
	afx_msg void SetMapRotationAngle(float nNewValue);
	afx_msg float GetMapRotationAngle(void);
	afx_msg CExtents* GetRotatedExtent(void);
	afx_msg CPointClass* GetBaseProjectionPoint(double rotPixX, double rotPixY);
	...
	afx_msg CImageClass* SnapShot2(LONG ClippingLayerNbr, DOUBLE Zoom, long pWidth);
	afx_msg CImageClass* SnapShot3(double left, double right, double top, double bottom, long Width);
	...
	afx_msg CMeasuring* GetMeasuring(void);
	afx_msg CShapeEditor* GetShapeEditor(void);
	...
	afx_msg CExtents* GetKnownExtents(tkKnownExtents extents);
	...
	afx_msg DOUBLE GeodesicLength(CShape* polyline);
	afx_msg DOUBLE GeodesicDistance(DOUBLE projX1, DOUBLE projY1, DOUBLE projX2, DOUBLE projY2);
	afx_msg DOUBLE GeodesicArea(CShape* polygon);
...
#pragma region EventMap
	...
	void FireMouseDown(short Button, short Shift, long x, long y)
		{/*SendMessage(eventidMouseDown,EVENT_PARAM(VTS_I2  VTS_I2  VTS_I4  VTS_I4), Button, Shift, x, y);*/}
...
#pragma region Members
	...
	CMeasuring* _measuring;
	CShapeEditor* _shapeEditor;
	CShapeEditor* _geodesicShape;
	CTiles* _tiles;						// the list of tiles (in-memory GDI+ bitmaps)
	...
	//	Projections
	// ---------------------------------------------
	CGeoProjection* _projection;			// must always have transformation to WWGS84
	CGeoProjection* _wgsProjection;		// must always have transform to current map projection (set in map.SetGeoProjection)
	CGeoProjection* _gmercProjection;		// must always have transform to current map projection (set in map.SetGeoProjection)
	CGeoProjection* _tileProjection;
	CGeoProjection* _tileReverseProjection;
...
public:
	CExtents* GetGeographicExtentsCore(bool clipForTiles, Extent* clipExtents = NULL);
	CTiles* GetTilesNoRef(void);
	bool SendMouseMove();
	...
	CGeoProjection* GetMapToWgs84Transform();
	CGeoProjection* GetMapToTilesTransform();
	CGeoProjection* GetWgs84ToMapTransform();
	CGeoProjection* GetTilesToMapTransform();
	CGeoProjection* GetWgs84Projection();
	CGeoProjection* GetGMercProjection();
	CGeoProjection* GetMapProjection();
...
private:	
	...
	//CPointClass* GetMeasuringPolyCenter(Gdiplus::PointF* data, int length);
	//void DrawMeasuringPolyArea(Gdiplus::Graphics* g, bool lastPoint, double lastGeogX, double lastGeogY, CPointClass* pnt);
	...
	CImageClass* SnapShotCore(double left, double right, double top, double bottom, long Width, long Height, 
	...
	bool DeserializeMapStateCore(CPLXMLNode* node, CStringW ProjectName, VARIANT_BOOL LoadLayers, CStopExecution* callback);
	CPLXMLNode* SerializeLayerCore(LONG LayerHandle, CStringW Filename);
	int DeserializeLayerCore(CPLXMLNode* node, CStringW ProjectName, bool utf8Filenames, CStopExecution* callback);		// adds new layer on loading (is used by map state)
	...
	void AttachGlobalCallbackToLayers(ILayer* object);
	...
	bool SplitByPolyline(long layerHandle, CShapefile* sf, vector<long>& indices, CShape* polyline);
	void ShowToolTipOnMouseMove(UINT nFlags, CPoint point);
	void HandleLButtonUpZoomBox(long vbflags, long x, long y);
	Extent GetPointSelectionBox(CShapefile* sf, double xProj, double yProj);
	...
	VARIANT_BOOL LayerIsIdentifiable(long layerHandle, CShapefile* sf);
	...
	long AddSingleLayer(ILayer* Object, BOOL pVisible);
...
public:
	...
	virtual CUndoList* _GetUndoList() { return _undoList; }
	virtual CShapefile* _GetShapefile(LONG layerHandle) { return GetShapefile(layerHandle); }
	virtual CShapeEditor* _GetShapeEditor() {return _shapeEditor; }
	...
	virtual CGeoProjection* _GetWgs84Projection() { return GetWgs84Projection(); }
	virtual CGeoProjection* _GetMapProjection() {return GetMapProjection(); }
	...
	virtual void _UnboundShapeFinished(CShape* shp);
