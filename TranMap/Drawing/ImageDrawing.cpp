...
ScreenBitmap* CImageDrawer::DrawImage(const CRect & rcBounds, CImageClass* iimg, bool returnBitmap)
{
	if( iimg == NULL )
		return NULL;
	...
	{	
		...
		if (type == USE_FILE_EXTENSION ) //|| type == BITMAP_FILE )
		{
			...
			else
			{
				...
				// ---------------------------------------------------------
				iimgNew = new CImageClass();

				VARIANT_BOOL vbretval;
				iimgNew->CreateNew(imgW, imgH, &vbretval);
				if (!vbretval)
				{
					delete iimgNew; iimgNew = NULL;   // error message in the image class
					delete[] pColours;
					goto final_clear;
				}
