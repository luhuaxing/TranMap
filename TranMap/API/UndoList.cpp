...
//*			GetShapefile()
//***********************************************************************/
CShapefile* CUndoList::GetShapefile(long layerHandle)
{
...
//*		get/put_Key()
//***********************************************************************
void  CUndoList::get_Key(BSTR *pVal)
{
...
void  CUndoList::put_Key(BSTR newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	::SysFreeString(_key);
	USES_CONVERSION;
...
//		get_LastErrorCode
// **********************************************************
void  CUndoList::get_LastErrorCode(long *pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	*pVal = _lastErrorCode;
...
//		get_ErrorMsg
// **********************************************************
void  CUndoList::get_ErrorMsg(long ErrorCode, BSTR *pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
		USES_CONVERSION;
...
{
	CShapefile* sf = GetShapefile(layerHandle);
	if (!sf) return false;
	long numShapes;
...
//		Add
// **********************************************************
void  CUndoList::Add(tkUndoOperation operation, LONG LayerHandle, LONG ShapeIndex, VARIANT_BOOL *retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
{
	CShape* shp = GetCurrentState(layerHandle, shapeIndex);
	item->SetShape(shp);
	...
	if (copyAttributes) {
		...
		if (sf) {
			...
			sf->get_Table(&tbl);
			if (tbl) {
				TableRow* row = tbl->CloneTableRow((int)shapeIndex);
				item->SetRow(row);
			}
			long category = -1;
...
//		Undo
// **********************************************************
void  CUndoList::Undo(VARIANT_BOOL zoomToShape, VARIANT_BOOL* retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	*retVal = VARIANT_FALSE;
	if (!CheckState()) return ;

	CShapeEditor* editor = _mapCallback->_GetShapeEditor();
...
//		Redo
// **********************************************************
void  CUndoList::Redo(VARIANT_BOOL zoomToShape, VARIANT_BOOL* retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	*retVal = VARIANT_FALSE;
	if (!CheckState()) return ;
...
{
	CShapefile* sf = NULL;
	sf = GetShapefile(item->LayerHandle);
	...
	switch (item->Operation)
	{
		...
		{	
			...
			if (vb) {
				TableRow* oldRow = table->SwapTableRow(item->Row, item->ShapeIndex);
				...
				if (editor && !item->WithinBatch) {
					CShape* shp = GetCurrentState(item->LayerHandle, item->ShapeIndex);
		...
		case uoAddShape:
		{
			...
			if (shp) {
				TableRow* row = table->CloneTableRow(item->ShapeIndex);
...
//		GetCurrentState
// **********************************************************
CShape* CUndoList::GetCurrentState(long layerHandle, long shapeIndex) 
{
	CShapeEditor* editor = _mapCallback->_GetShapeEditor();
...
//		get_UndoCount
// **********************************************************
void  CUndoList::get_UndoCount(LONG* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		get_RedoCount
// **********************************************************
void  CUndoList::get_RedoCount(LONG* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		get_TotalLength
// **********************************************************
void  CUndoList::get_TotalLength(LONG* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	int lastId = EMPTY_BATCH_ID;
	long count = 0;
...
//		BeginBatch
// **********************************************************
void  CUndoList::BeginBatch(VARIANT_BOOL* retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		EndBatch
// **********************************************************
void  CUndoList::EndBatch(LONG* retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (_batchId == EMPTY_BATCH_ID) {
		*retVal = -1;
...
//		GetLastId
// **********************************************************
void  CUndoList::GetLastId(LONG* retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		ShortcutKey
// **********************************************************
void  CUndoList::get_ShortcutKey(tkUndoShortcut* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	*pVal = _shortcutKey;
	return ;
}
void  CUndoList::put_ShortcutKey(tkUndoShortcut newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	_shortcutKey = newVal;
	return ;
...
//		ClearForLayer
// **********************************************************
void  CUndoList::ClearForLayer(LONG LayerHandle)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	bool changed = false;
