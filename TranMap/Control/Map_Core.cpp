...
// ****************************************************************
//ajp (June 2010)
CPointClass* CMapView::GetBaseProjectionPoint(double rotPixX, double rotPixY)
{
  CPointClass *curPoint = new CPointClass();
...
// ****************************************************************
//ajp (June 2010)
CExtents* CMapView::GetRotatedExtent()
{
	Extent rotExtent;
...
VARIANT_BOOL CMapView::SetImageLayerColorScheme(LONG LayerHandle, IDispatch* ColorScheme)
{
	...
	if(IS_VALID_LAYER(LayerHandle,_allLayers))
	{
		...
		// lu ColorScheme->QueryInterface(IID_IGridColorScheme,(void**)&scheme);
		if (scheme)
		{
			CImageClass* img = this->GetImage(LayerHandle);
			if (img != NULL)
			{
...
//		GetColorScheme()
// *********************************************************
CGridColorScheme* CMapView::GetColorScheme(long LayerHandle)
{
	if( IS_VALID_LAYER(LayerHandle,_allLayers) )
	{	
		Layer * l = _allLayers[LayerHandle];
