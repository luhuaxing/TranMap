...
//		CloneField()
// ********************************************************************
int CloneField(CShapefile* source, CShapefile* target, int fieldIndex, long newFieldIndex)
{
	CField* fld = NULL;
...
void CShapefile::InsertShapesVector(CShapefile* sf, vector<CShape* >& vShapes, 
									CShapefile* sfSubject, long subjectId, std::map<long, long>* fieldMapSubject,
									CShapefile* sfClip, long clipId, std::map<long, long>* fieldMapClip)
{
...
bool InsertGeosGeometry(CShapefile* sfTarget, GEOSGeometry* gsNew, CShapefile* sfSouce, int initShapeIndex )
{
	if (gsNew)
	{
		ShpfileType shpType;
...
void CopyShape(CShapefile* sfSource, CShape* shp, CShapefile* sfResult)
{
	if (shp)
	{
		USES_CONVERSION;
...
void  CShapefile::SelectByShapefile(CShapefile* sf, tkSpatialRelation Relation, 
											VARIANT_BOOL SelectedOnly,  VARIANT* arr, ICallback* cBack, VARIANT_BOOL *retval)
{
	...
	*retval = VARIANT_FALSE;
	...
	if (_numShapes1 == 0)return ;
	if (_numShapes2 == 0) return ;
...
VARIANT_BOOL CShapefile::SelectShapesAlt(CExtents *BoundBox, double Tolerance, SelectMode SelectMode, VARIANT* arr)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void  CShapefile::Dissolve(long FieldIndex, VARIANT_BOOL SelectedOnly, CShapefile** sf)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//     DissolveWithStats()
// *************************************************************
void  CShapefile::DissolveWithStats(long FieldIndex, VARIANT_BOOL SelectedOnly, CFieldStatOperations* statOperations, CShapefile** sf)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//  Merges shapes of the shapefile based on the attribute of the given
// 	field. Shapes with the same attribute are merged into one.
void CShapefile::DissolveCore(long FieldIndex, VARIANT_BOOL SelectedOnly, CFieldStatOperations* operations, CShapefile** sf)
{
	// ----------------------------------------------
	//   Validation
	// ----------------------------------------------
	...
	{
		DissolveGEOS(FieldIndex, SelectedOnly, operations, *sf);
	}
	else
	{
		DissolveClipper(FieldIndex, SelectedOnly, operations, *sf);
	}
...
//     CalculateFieldStats()
// *************************************************************
void CShapefile::CalculateFieldStats(map<int, vector<int>*>& fieldMap, CFieldStatOperations* ioperations, CShapefile* sf)
{
	// --------------------------------------------
	...
	for(size_t i = 0; i < operations->size(); i++)
	{
		...
		if (op->valid)
		{
			...
			if (field)
			{
				...
				name =  name + L"_" + A2W(GetStatOperationName(op->operation));
...
//     DissolveGEOS()
// *************************************************************
void CShapefile::DissolveGEOS(long FieldIndex, VARIANT_BOOL SelectedOnly, CFieldStatOperations* operations, CShapefile* sf)
{
...
//     DissolveClipper()
// *************************************************************
void CShapefile::DissolveClipper(long FieldIndex, VARIANT_BOOL SelectedOnly,  CFieldStatOperations* operations, CShapefile* sf)
{
	map <int, vector<int>*> fieldMap;  			    // index in output, indices in input
	map <CComVariant, vector<int>*> indicesMap;		// value in input, indices in input
...
// ********************************************************************
void  CShapefile::AggregateShapesWithStats(VARIANT_BOOL SelectedOnly, LONG FieldIndex, CFieldStatOperations* statOperations, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		AggregateShapes()
// ********************************************************************
void  CShapefile::AggregateShapes(VARIANT_BOOL SelectedOnly, LONG FieldIndex, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CShapefile::AggregateShapesCore(VARIANT_BOOL SelectedOnly, LONG FieldIndex, CFieldStatOperations* operations, CShapefile** retval)
{
	long numFields;
	this->get_NumFields(&numFields);
	...
	map <int, vector<int>*> fieldMap;  			    // index in output, indices in input
	map <CComVariant, vector<int>*> indicesMap;		// value in input, indices in input
	map <CComVariant, vector<CShape*>*> shapeMap;
	...
	for(long i = 0; i < size; i++)
	{
		...
		{
			vector<CShape*>* v = new vector<CShape*>;
			v->push_back(shp);
			shapeMap[val] = v;
	...
	percent = 0;
	size = shapeMap.size();
	map <CComVariant, vector<CShape*>*>::iterator p = shapeMap.begin();
...
//		BufferByDistance()
// ********************************************************************
void  CShapefile::BufferByDistance(double Distance, LONG nSegments, VARIANT_BOOL SelectedOnly, VARIANT_BOOL MergeResults, CShapefile** sf)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
			...
			if (oGeom)
			{
				...
				if (type == wkbMultiPolygon || type == wkbMultiPolygon25D)
				{
					...
					if (OgrConverter::MultiPolygon2Polygons(oGeom, &polygons))
					{
						...
						{
							CShape* shp = OgrConverter::GeometryToShape(polygons[i], isM);
							if (shp)
							{
				...
				else
				{
					...
					CShape* shp = OgrConverter::GeometryToShape(oGeom, isM);
					if (shp)
					{
						(*sf)->EditInsertShape(shp, &count, &vb);	
						delete shp;
...
//		GetDifference()
// ********************************************************************
void  CShapefile::Difference(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		Clip()
// ********************************************************************
void  CShapefile::Clip(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
// *	GetIntersection()				             
// **********************************************************************
void  CShapefile::GetIntersection(VARIANT_BOOL SelectedOnlyOfThis, CShapefile* sf,
										 VARIANT_BOOL SelectedOnly, ShpfileType fileType, ICallback* cBack, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		GetSymmDifference()
// ********************************************************************
void  CShapefile::SymmDifference(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		GetUnion()
// ********************************************************************
void  CShapefile::Union(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		DoClipOperarion()
// ********************************************************************
void CShapefile::DoClipOperation(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, 
								 VARIANT_BOOL SelectedOnlyOverlay, CShapefile** retval, 
								 tkClipOperation operation, ShpfileType returnType)
{
	...
	CShapefile* sfCopy = (operation == clIntersection || operation == clSymDifference || operation == clUnion)? sfOverlay : NULL;
...
//		ClipGEOS()
// ********************************************************************
void CShapefile::ClipGEOS(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile* sfResult)
{
...
//		ClipClipper()
// ********************************************************************
void CShapefile::ClipClipper(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, CShapefile* sfResult)
{
	QTree* qTree = ((CShapefile*)sfOverlay)->GetTempQTree();
	...
	for(long subjectId =0; subjectId < numShapesSubject; subjectId++)		
	{
		...
		if (shapeIds.size() > 0)
		{
			...
			if (poly1)
			{
				...
				{
					CShape* shp = ogr.ClipperPolygon2Shape(&polyResult);
					if (shp)
					{
						vector<CShape* > vShapes;
...
void CShapefile::IntersectionGEOS(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfClip, VARIANT_BOOL SelectedOnlyClip, 
								  CShapefile* sfResult, map<long, long>* fieldMap, 
...
// ******************************************************************
CShapefile* CShapefile::IntersectionClipperNoAttributes(VARIANT_BOOL SelectedOnlySubject, CShapefile* sfClip, VARIANT_BOOL SelectedOnlyClip )
{
	if (!sfClip)
		return NULL;

	CShapefile* sfResult = NULL;
	this->Clone(&sfResult);
	...
	if (clp.Execute(ClipperLib::ClipType::ctIntersection, polyResult)) {
		...
		CShape* shp = converter.ClipperPolygon2Shape(&polyResult);
...
//		IntersectionClipper()
// ******************************************************************
void CShapefile::IntersectionClipper( VARIANT_BOOL SelectedOnlySubject, CShapefile* sfClip, VARIANT_BOOL SelectedOnlyClip, 
									  CShapefile* sfResult, map<long, long>* fieldMap,
									  std::set<int>* subjectShapesToSkip, 
								      std::set<int>* clippingShapesToSkip)
{
	...
	CGeoProjection* projection = NULL;
...
//		DifferenceGEOS()
// ********************************************************************
void CShapefile::DifferenceGEOS(CShapefile* sfSubject, VARIANT_BOOL SelectedOnlySubject, CShapefile* sfOverlay, VARIANT_BOOL SelectedOnlyOverlay, 
								CShapefile* sfResult, map<long, long>* fieldMap, set<int>* shapesToSkip)
{
	QTree* qTree = ((CShapefile*)sfOverlay)->GetTempQTree();
...
//		DifferenceClipper()
// ******************************************************************
void CShapefile::DifferenceClipper(CShapefile* sfSubject, VARIANT_BOOL SelectedOnlySubject, CShapefile* sfClip, VARIANT_BOOL SelectedOnlyClip, 
								   CShapefile* sfResult, map<long, long>* fieldMap, set<int>* shapesToSkip)
{
	QTree* qTree = ((CShapefile*)sfClip)->GetTempQTree();
...
//		GeometryEngine()
// ********************************************************************
void  CShapefile::get_GeometryEngine(tkGeometryEngine* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void  CShapefile::put_GeometryEngine(tkGeometryEngine newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		PointInShape()
// ********************************************************************
void  CShapefile::PointInShape(LONG ShapeIndex, DOUBLE x, DOUBLE y, VARIANT_BOOL* retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{	
		...
		CShape * shp = _shapeData[ShapeIndex]->shape;
		shp->get_NumParts(&numParts);
		shp->get_NumPoints(&numPoints);
...
//		PointInShapefile()
// ********************************************************************
void  CShapefile::PointInShapefile(DOUBLE x, DOUBLE y, LONG* ShapeIndex)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
// ********************************************************************
void  CShapefile::BeginPointInShapefile(VARIANT_BOOL* retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (_writing)
	{
...
//		EndPointInShapefile()
// ********************************************************************
void  CShapefile::EndPointInShapefile(void)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		ExplodeShapes()
// ********************************************************************
void  CShapefile::ExplodeShapes(VARIANT_BOOL SelectedOnly, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	for (long i = 0; i < numShapes; i++)
	{
		...
		if (((CShape*)shp)->ExplodeCore(vShapes))
		{
			for (long j = 0; j < (long)vShapes.size(); j++)
			{
				...
				delete vShapes[j];	// reference was added in EditInsertShape
			}
		}
	}
...
//		ExportSelection()
// ********************************************************************
void  CShapefile::ExportSelection(CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		Sort()
// ********************************************************************
void  CShapefile::Sort(LONG FieldIndex, VARIANT_BOOL Ascending, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	USES_CONVERSION;
	...
	long numShapes = _shapeData.size();		
	multimap <CComVariant, CShape*> shapeMap;
	CComVariant val;	
	...
	for (long i = 0; i < numShapes; i++)
	{
		...
		if (shp)
		{
			...
			this->get_CellValue(FieldIndex, i, &val);	
			pair<CComVariant, CShape*> myPair(val, shp);	
	...
	if (Ascending)
	{
		multimap <CComVariant, CShape*>::iterator p;
		p = shapeMap.begin();
	...
	{
		std::multimap <CComVariant, CShape*>::reverse_iterator p = shapeMap.rbegin();
...
//		Merge()
// ********************************************************************
void  CShapefile::Merge(VARIANT_BOOL SelectedOnlyThis, CShapefile* sf, VARIANT_BOOL SelectedOnly, CShapefile** retval)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	USES_CONVERSION;
...
//		SimplifyLines()
// **********************************************************************
void  CShapefile::SimplifyLines(DOUBLE Tolerance, VARIANT_BOOL SelectedOnly, CShapefile** retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	USES_CONVERSION;
...
//		Segmentize()
// **********************************************************************
void  CShapefile::Segmentize(CShapefile** retVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
