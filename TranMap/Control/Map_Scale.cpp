...
VARIANT_BOOL CMapView::ZoomToTileLevel(int zoom)
{
	...
	else
	{
		...
		CGeoProjection* projTemp = GetTilesToMapTransform();
		if (projTemp)
		{
...
//	   SetExtents()
// ****************************************************
void CMapView::SetExtents(CExtents* newValue)
{
	if( !newValue )
	{	
...
// *****************************************************
VARIANT_BOOL CMapView::SetGeographicExtents(CExtents* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (!pVal)
	{
	...
	else
	{
		...
		else if (_transformationMode == tmDoTransformation)
		{
			...
			CGeoProjection* p = GetWgs84ToMapTransform();
			if (p) {
...
//		GetMeasuring()
// *****************************************************
CMeasuring* CMapView::GetMeasuring() 
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	return _measuring;
}
...
//		GetShapeEditor()
// *****************************************************
CShapeEditor* CMapView::GetShapeEditor() 
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	return _shapeEditor;
}
...
// *****************************************************
CExtents* CMapView::GetGeographicExtents()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
// ***************************************************************
CExtents* CMapView::GetGeographicExtentsCore(bool clipForTiles, Extent* clipExtents)
{
	CExtents * box = NULL;
	...
	if (GetMapProjection())
	{
		...
		{
			VARIANT_BOOL vb;
			CGeoProjection* projTemp = GetMapToWgs84Transform();
			if (projTemp)
			{
				Extent ext;
...
VARIANT_BOOL CMapView::ZoomToSelected(LONG LayerHandle)
{
	...
	CShapefile* sf = this->GetShapefile(LayerHandle);
	if (sf)
	{
		long numShapes;
...
//		GetMaxExtents()
// ****************************************************************
CExtents* CMapView::GetMaxExtents(void)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//		ZoomToKnownExtents()
// ****************************************************************
CExtents* CMapView::GetKnownExtents(tkKnownExtents extents)
{
	CExtents* box = new CExtents();
...
bool CMapView::GetGeoPosition(double& x, double& y)
{
	...
	switch(_transformationMode)
	{
		...
		case tmDoTransformation:
			{
				VARIANT_BOOL vb;
				CGeoProjection* p = GetMapToWgs84Transform();
				if (p) {
...
{
	if (_transformationMode != tmNotDefined) {
		CExtents* ext = GetKnownExtents(extents);
		if (ext) {
	...
	switch(_transformationMode)
	{
		...
		case tmWgs84Complied:
			...
			*projY = degreesLatY;
			return VARIANT_TRUE;
		case tmDoTransformation:
			CGeoProjection* gp = GetWgs84ToMapTransform();
			VARIANT_BOOL vb;
