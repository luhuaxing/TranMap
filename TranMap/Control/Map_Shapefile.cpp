...
//		GetUDPointType()
// *****************************************************************
CImageClass* CMapView::GetUDPointType(long LayerHandle)
{
	CDrawingOptionsEx* options = get_ShapefileDrawingOptions(LayerHandle);
	if (options)
	{
...
void CMapView::SetUDPointType(long LayerHandle, LPDISPATCH newValue)
{
	...
	if (options)
	{
		...
		// lu newValue->QueryInterface( IID_IImage, (void**)&iimg );
		options->picture = iimg;
		if (iimg)
		{
			delete iimg;
		}
...
VARIANT_BOOL CMapView::FindSnapPoint(double tolerance, double xScreen, double yScreen, double* xFound, double* yFound)
{
	...
	CShapefile* foundShapefile = NULL;
...
//		LayerIsIdentifiable
// ************************************************************
VARIANT_BOOL CMapView::LayerIsIdentifiable(long layerHandle, CShapefile* sf)
{
...
bool CMapView::DrillDownSelect(double projX, double projY, long& layerHandle, long& shapeIndex )
{
	...
	{
		CShapefile* sf = GetShapefile(handles[i]);
		if (sf) {
...
LayerShape CMapView::FindShapeAtProjPoint(double prjX, double prjY, std::vector<int>& layers)
{
	...
	{
		CShapefile* sf = GetShapefile(layers[i]);
		if (sf) {
			double tol = 0.0;
...
{
	CShapefile* sf = GetShapefile(info.LayerHandle);
	if (sf) {
		CShape* shape = NULL;
		sf->get_Shape(info.ShapeIndex, &shape);
		if (shape) 
		{
			CShape* shpClone = NULL;
			...
			CShapeDrawingOptions* options = ShapeStyleHelper::GetHotTrackingStyle(sf, color, m_cursorMode == cmIdentify);
			if (options) {
...
void CMapView::ClearLabelFrames()
{
	...
	for (int i = 0; i < (int)_activeLayers.size(); i++)
	{
		...
		{	
			...
			// labels
			CLabels* LabelsClass = l->get_Labels();
