...
bool tkGridRaster::GetIntValueGridColorTable(CGridColorScheme ** newscheme)
{
	...
	*newscheme = new CGridColorScheme();
	...
	for (int i = iMin; i < iMax; i++)
	{
		...
		if (newColor != lastColor && i != iMin)
		{
			...
			CGridColorBreak * newbreak = new CGridColorBreak();
...
bool tkGridRaster::LoadRasterCore(char* filenameA, bool InRam, GridFileType fileType)
{
	__try
	{
		...
		if( rasterDataset->GetGeoTransform( adfGeoTransform ) == CE_None )
		{
			...
			// it PixelIsPoint for TranMap compatibility. Saving out won't hurt
